...
class StoreService: ObservableObject {
    ...
    func fetchOrders() {
        ...
        ordersListener = db.collection("users").document(uid).collection("orders")
            .addSnapshotListener { [weak self] querySnapshot, error in
                ...
                // 2. Sync Notifications with Changes (Background)
                querySnapshot?.documentChanges.forEach { diff in
                    if let order = try? diff.document.data(as: CakeOrder.self) {
                        
                        // OPTIMIZATION: Combine date & time string
                        let dateTimeStr = "\(order.pickupDate) \(order.pickupTime)"
                        let formatter = DateFormatter()
                        formatter.dateFormat = "yyyy-MM-dd HH:mm"
                        formatter.locale = Locale(identifier: "en_US_POSIX")
                        formatter.timeZone = TimeZone.current
                        
                        // If we can parse the date, check if it's "relevant" (future or < 24h old)
                        if let pickupDate = formatter.date(from: dateTimeStr) {
                            let twentyFourHoursAgo = Date().addingTimeInterval(-86400) // 24 hours in seconds
                            
                            // Skip processing for orders older than 24 hours
                            if pickupDate < twentyFourHoursAgo {
                                return
                            }
                        }
                        ...
                        case .added, .modified:
                            if order.status == "pending" {
                                NotificationManager.shared.scheduleForOrder(order)
                            } else {
                                NotificationManager.shared.cancelForOrder(order)
                            }
                        case .removed:
                            NotificationManager.shared.cancelForOrder(order)
                        }
    ...
    func addOrder(_ order: CakeOrder) {
        ...
        do {
            ...
            // self.orders.append(order) // Moving this after getting ID
            
            let ref = try db.collection("users").document(uid).collection("orders").addDocument(from: order)
            print("DEBUG: Added order with ID: \(ref.documentID)")
            
            var orderWithId = order
            orderWithId.id = ref.documentID
            
            // Update local array with the ID-bearing order
            self.orders.append(orderWithId)
            
            // Schedule Notifications with the correct ID
            NotificationManager.shared.scheduleForOrder(orderWithId)
    ...
    func updateOrderStatus(orderId: String, status: String) {
        ...
        if let order = self.orders.first(where: { $0.id == orderId }) {
            if status == "completed" {
                NotificationManager.shared.cancelForOrder(order)
    ...
    func deleteOrder(orderId: String) {
        ...
        // We construct a temporary order object with just the ID to cancel notifications
        // strictly speaking cancelForOrder only needs the ID
        let tempOrder = CakeOrder(id: orderId, itemName: "", customerName: "", quantity: 0, total: 0, notes: "", source: "", timestamp: "", pickupDate: "", pickupTime: "", status: "")
        NotificationManager.shared.cancelForOrder(tempOrder)
    }
